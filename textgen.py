...
root_path = os.path.dirname(os.path.realpath('./HarryPotter.txt'))
print(root_path)
...
def create_dataset():
  # path to dataset
  dataset_path = os.path.join(root_path, r"./HarryPotter.txt")

  # Read, then decode for py2 compat.
  text = open(dataset_path, 'rb').read().decode(encoding='unicode_escape')
  #text = open(dataset_path, 'rb').read()
...
if __name__ == '__main__':
    ...
    if args.command == 'generate':
      ...
      dataset, vocab, idx2char, char2idx = create_dataset()
      # Get the model path
      model_path = os.path.join('./logs/G25N7PV4WK07OIW/', 'model.h5')
      # Length of the vocabulary in chars
      vocab_size = len(vocab)
      # Build the model with the dataset generated earlier
      model = build_model(vocab_size, embedding_dim, rnn_units, batch_size=1)
      ...
      f.write(generate_text(model, idx2char, char2idx,start_string=u"Harry"))
      f.close()
      
      #open and read the file after the appending:
      f = open("./generated_text.txt", "r")
      print(f.read())
      #print(generate_text(model, idx2char, char2idx,start_string=u"Harry"))
